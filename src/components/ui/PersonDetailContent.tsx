...
export default function PersonDetailContent({
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className="w-full min-w-0 max-w-full space-y-8 overflow-x-hidden"
    >
      {/* ① 人像 + 名字 + 标签 */}
      ...
      {/* ④ 为什么她值得被看见：保留原详情中的核心策展信息 */}
      <motion.section variants={itemVariants} aria-labelledby="visible-title">
        <h2
          id="visible-title"
          className={cn(
            "flex items-center gap-2 font-serif font-semibold mb-4",
            sizes.sectionTitle
          )}
        >
          <Star className="h-4 w-4 fill-secondary/30 text-secondary" />
          为什么她值得被看见
        </h2>
            ...
            {echoes.map((c) => {
              ...
              return (
                <button
                  ...
                  className="dc-conn-card w-full min-w-0 max-w-full overflow-hidden text-left p-4 rounded-xl border border-border hover:border-secondary/50 bg-card/60 disabled:cursor-default group"
                ...
                >
                  ...
                  <p className="break-words text-xs leading-relaxed text-muted-foreground italic pt-2.5 border-t border-dashed border-border">
